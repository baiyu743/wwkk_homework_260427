...
class LoginWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.title_label = QLabel('学生学籍管理系统登录', self)
...
#cursor.fetchone() 方法返回查询结果的第一条记录。
    ...
    def open_teacher_dashboard(self, teacher_username):
        ...
        self.teacher_window.show()#TeacherDashboard 类，传递一个名为 teacher_username 的参数，并显示该界面。
...
class TeacherDashboard(QWidget):
    #teacher_username：这个参数传递给 TeacherDashboard 类，用于显示当前教师的用户名。这样在界面中可以显示“欢迎您，xx老师”。
...
#初始化背景
    ...
    #initUI 方法负责界面上的控件布局。它创建了多个按钮，分别用于不同的操作，比如“成绩录入”、“成绩查询”、“学生管理”等。
    def initUI(self):
        layout = QVBoxLayout()
        ...
        self.add_grade_button.clicked.connect(self.add_grade)#将按钮的点击事件连接到相应的方法
