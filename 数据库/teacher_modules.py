...
class GradeSearchWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.search_button.clicked.connect(self.search_grade)
    ...
    def search_grade(self):
        ...
        try:
            ...
            if grade_data and len(grade_data) > 0:
                result_msg = ""
                for idx, (s_id, c_id, grade) in enumerate(grade_data, 1):
                    result_msg += f"第{idx}条记录：\n学生ID：{s_id}\n课程ID：{c_id}\n成绩：{grade}\n\n"
        ...
        try:
            ...
            if student:
                ...
                for col_idx, col_data in enumerate(student):
                    ...
                    self.student_table.setItem(0, col_idx, item)
...
if __name__ == '__main__':
    ...
    # 传入测试教师ID，实际使用时替换为登录的教师ID
    main_win = TeacherMainWindow(teacher_id="T001")
    main_win.show()
    sys.exit(app.exec_())